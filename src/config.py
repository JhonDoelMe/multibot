...
import logging
import sys # Додамо sys для можливого виходу

# Завантажуємо .env в першу чергу, щоб змінні з нього були доступні одразу
load_dotenv()

logger = logging.getLogger(__name__) # Логгер буде налаштований в __main__.py
...
WEATHERAPI_COM_KEY = os.getenv("WEATHERAPI_COM_KEY") # КЛЮЧ ДЛЯ WEATHERAPI.COM

# --- Налаштування Вебхука ---
WEBHOOK_BASE_URL = os.getenv("WEBHOOK_BASE_URL")
WEBHOOK_PATH = os.getenv("WEBHOOK_PATH", f"/webhook/{BOT_TOKEN[:10] if BOT_TOKEN else 'default_bot_path'}") # Додано перевірку BOT_TOKEN
...
INITIAL_DELAY = int(os.getenv("INITIAL_DELAY", 1)) # секунди
API_REQUEST_TIMEOUT = int(os.getenv("API_REQUEST_TIMEOUT", 15)) # Таймаут для одного запиту

# Загальний таймаут для сесії aiohttp, якщо потрібно (може перекривати API_REQUEST_TIMEOUT)
# Наприклад, config.API_SESSION_TOTAL_TIMEOUT для `aiohttp.ClientTimeout(total=...)`
API_SESSION_TOTAL_TIMEOUT = int(os.getenv("API_SESSION_TOTAL_TIMEOUT", 30))
API_SESSION_CONNECT_TIMEOUT = int(os.getenv("API_SESSION_CONNECT_TIMEOUT", 10)) # Таймаут на з'єднання

# --- Налаштування кешування (aiocache) ---
CACHE_BACKEND = os.getenv("CACHE_BACKEND", "memory") # 'memory' або 'redis'
CACHE_REDIS_URL = os.getenv("CACHE_REDIS_URL", "redis://localhost:6379/0")

CACHE_TTL_ALERTS = int(os.getenv("CACHE_TTL_ALERTS", 60)) # seconds
...
CACHE_TTL_WEATHER_BACKUP = int(os.getenv("CACHE_TTL_WEATHER_BACKUP", 700)) # WeatherAPI.com
CACHE_TTL_CURRENCY = int(os.getenv("CACHE_TTL_CURRENCY", 3600))
CACHE_TTL_REGIONS = int(os.getenv("CACHE_TTL_REGIONS", 86400)) # для списку регіонів (якщо є)

SENTRY_DSN = os.getenv("SENTRY_DSN")
THROTTLING_RATE_DEFAULT = float(os.getenv("THROTTLING_RATE_DEFAULT", 0.7)) # секунди між запитами

# --- Критичні перевірки ---
if not BOT_TOKEN:
    critical_error_msg = "CRITICAL ERROR: BOT_TOKEN is not defined in environment variables. Bot cannot start."
    # Логування може ще не бути налаштоване тут, тому print до stderr
...
# Функція для логування статусу конфігурації (буде викликана з __main__.py після налаштування логера)
def log_config_status():
    ...
    # API ключі - попередження, якщо не встановлені
    service_keys_status = {
