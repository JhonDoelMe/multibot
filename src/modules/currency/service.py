...
async def get_pb_exchange_rates(bot: Bot, cash: bool = True) -> Optional[List[Dict[str, Any]]]:
    ...
    return None # Если цикл завершился без успеха (хотя не должен)
...
def format_rates_message(rates: List[Dict[str, Any]], rate_type_name: str) -> str:
    ...
    for rate in rates:
        ccy = rate.get('ccy'); buy = float(rate.get('buy', 0)); sale = float(rate.get('sale', 0))
        buy_str = f"{buy:.2f}"; sale_str = f"{sale:.2f}"
