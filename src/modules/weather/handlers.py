...
async def _get_and_show_weather(
    ...
    # ... (код функции без изменений, он корректно сохраняет current_shown_city в state) ...
    ...
    if weather_data and weather_data.get("cod") == 200:
        ...
        await state.update_data(city_to_save=actual_city_name_from_api, city_display_name=city_display_name, current_shown_city=user_city_input)
        if not is_preferred:
            ...
            except Exception: await message_to_edit_or_answer.answer(text_to_send, reply_markup=reply_markup)
            ...
            except Exception: await message_to_edit_or_answer.answer(weather_message, reply_markup=reply_markup)
...
async def weather_entry_point(target: Union[Message, CallbackQuery], state: FSMContext, session: AsyncSession):
    ...
    if preferred_city:
        ...
        except Exception as e: logger.error(f"Error editing/sending message in weather_entry_point: {e}"); await message_to_edit_or_answer.answer(text, reply_markup=reply_markup)
...
async def handle_action_other_city(callback: CallbackQuery, state: FSMContext):
    # ... (код без изменений) ...
     logger.info(f"User {callback.from_user.id} requested OTHER city from weather view."); await callback.message.edit_text("🌍 Будь ласка, введіть назву іншого міста:", reply_markup=get_weather_enter_city_back_keyboard()); await state.set_state(WeatherStates.waiting_for_city); await callback.answer()
...
async def handle_action_refresh(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    # ... (код без изменений) ...
     ...
     if current_city: logger.info(f"User {user_id} requested REFRESH for city: {current_city}"); is_preferred_city = (preferred_city is not None and preferred_city.lower() == current_city.lower()); await _get_and_show_weather(callback, state, session, user_city_input=current_city, is_preferred=is_preferred_city)
     else: logger.warning(f"User {user_id} requested REFRESH, but no city found in state."); await callback.message.edit_text("Не вдалося визначити місто для оновлення. Введіть назву:", reply_markup=get_weather_enter_city_back_keyboard()); await state.set_state(WeatherStates.waiting_for_city); await callback.answer("Не вдалося оновити.", show_alert=True)
...
async def handle_save_city_yes(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    if not city_to_save_in_db or not city_display_name:
        ...
        # await state.clear() # Убрали очистку здесь
    ...
    if db_user:
         try:
            ...
            await callback.message.edit_text(text, reply_markup=reply_markup)
             ...
             logger.exception(f"Database error while saving city for user {user_id}: {e}"); await session.rollback(); await callback.message.edit_text("😥 Виникла помилка...")
    else: # ... (обработка ненайденного пользователя) ...
         logger.error(f"Cannot save city for user {user_id}: user not found in DB."); await callback.message.edit_text("Помилка: не вдалося знайти дані...")

    # await state.clear() # <<< УБИРАЕМ ОЧИСТКУ СОСТОЯНИЯ ЗДЕСЬ
