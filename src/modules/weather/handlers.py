# src/modules/weather/handlers.py (Исправлен SyntaxError)
...
async def _get_and_show_weather(
    ...
    if coords: request_details = f"coords ({coords['lat']:.4f}, {coords['lon']:.4f})"; logger.info(f"User {user_id} req weather by {request_details}"); weather_data = await get_weather_data_by_coords(coords['lat'], coords['lon']); is_preferred = False
    elif city_input: # Определение is_preferred
         request_details = f"city '{city_input}'"; logger.info(f"User {user_id} req weather by {request_details}"); weather_data = await get_weather_data(city_input)
    ...
    if weather_data and (weather_data.get("cod") == 200 or str(weather_data.get("cod")) == "200"):
        ...
        reply_markup = None; text_to_send = weather_message
        if ask_to_save: text_to_send += f"\n\n💾 Зберегти <b>{city_display_name}</b> як основне місто?"; reply_markup = get_save_city_keyboard(); await state.set_state(WeatherStates.waiting_for_save_decision)
        else: reply_markup = get_weather_actions_keyboard()

        # Отправка/Редактирование финального сообщения
        try:
            ...
            # --- ИСПРАВЛЕНО ЗДЕСЬ ---
            # Новый блок try для отправки нового сообщения
            try:
...
# --- Остальные хэндлеры (weather_entry_point, handle_location, handle_city_input, etc.) ---
# --- остаются без изменений по сравнению с версией из ответа #110 ---
# ... (весь остальной код файла handlers.py) ...
...
async def handle_location(message: Message, state: FSMContext, session: AsyncSession):
     if message.location: lat = message.location.latitude; lon = message.location.longitude; user_id = message.from_user.id; logger.info(f"Received location from user {user_id}: lat={lat}, lon={lon}"); await state.clear(); await _get_and_show_weather(message, state, session, coords={"lat": lat, "lon": lon})

@router.message(WeatherStates.waiting_for_city)
async def handle_city_input(message: Message, state: FSMContext, session: AsyncSession):
     user_city_input = message.text.strip(); await _get_and_show_weather(message, state, session, city_input=user_city_input)
...
async def handle_action_refresh(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
     ...
     if coords: logger.info(f"User {user_id} requested REFRESH for coords: {coords}"); await _get_and_show_weather(callback, state, session, coords=coords)
     elif city_name: logger.info(f"User {user_id} requested REFRESH for city: {city_name}"); await _get_and_show_weather(callback, state, session, city_input=city_name)
     else: logger.warning(f"User {user_id} requested REFRESH, no location/city in state."); await callback.message.edit_text("..."); await state.set_state(WeatherStates.waiting_for_city); await callback.answer("...", show_alert=True)
...
async def handle_show_current_weather(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
     ...
     if current_city: logger.info(f"User {user_id} requested back to current weather: {current_city}"); await _get_and_show_weather(callback, state, session, city_input=current_city) # <<< Исправлен вызов
