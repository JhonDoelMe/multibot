...
async def _get_and_show_weather(
    ...
    is_preferred = False
    request_details = ""
    is_coords_request_flag = False # Флаг для передачи в format_weather_message
    ...
    if coords:
        is_coords_request_flag = True # Устанавливаем флаг
        ...
        is_coords_request_flag = False # Сбрасываем флаг
    ...
    if weather_data and str(weather_data.get("cod")) == "200":
        ...
        if coords and actual_city_name_from_api:
             # Для координат display name уже содержит "Прогноз за вашими координатами..."
             # поэтому format_weather_message сам разберется с заголовком через флаг is_coords_request
            city_display_name_for_user_message = actual_city_name_from_api # Передаем чистое имя города от API
        elif coords:
            city_display_name_for_user_message = "ваші координати" # Если API не вернуло имя
        ...
        # Передаем флаг is_coords_request_flag в format_weather_message
        ...
        state_data_to_update = {
            ...
            "city_display_name": city_display_name_for_user_message, # Сохраняем то, что будет в заголовке или вопросе о сохранении
            "current_shown_city": current_shown_city_for_refresh_fsm,
            "current_coords": coords,
            "preferred_city_from_db": preferred_city_from_db,
            "is_coords_request": is_coords_request_flag # Сохраняем флаг в состояние FSM
        }
        ...
        if ask_to_save:
            # Для вопроса о сохранении используем чистое имя города, если оно есть от API, или то, что ввел юзер
...
# Остальные хендлеры остаются без изменений, так как проблема была в service.py и передаче параметров в format_weather_message
...
async def handle_forecast_request(callback: CallbackQuery, state: FSMContext, session: AsyncSession, bot: Bot):
    ...
    # Для заголовка прогноза используем city_display_name, так как он уже отформатирован (например, с "Прогноз за ...")
    # если current_coords есть в стейте. Или используем city_name_for_api_request, если display_name не найден.
    is_coords_forecast = user_data.get("is_coords_request", False)
    display_name_for_forecast_header = user_data.get("city_display_name", city_name_for_api_request)
    
    if not city_name_for_api_request: # Должен быть город (от API или пользовательский ввод) для запроса прогноза
        logger.warning(f"User {user_id} requested forecast, but 'current_shown_city' (for API request) not found in FSM state. Data: {user_data}")
        await callback.answer("Спочатку отримайте поточну погоду для міста.", show_alert=True)
        return
    ...
    if forecast_api_data and str(forecast_api_data.get("cod")) == "200":
        # Передаем display_name_for_forecast_header в format_forecast_message для корректного заголовка
        ...
        logger.info(f"User {user_id}: Sent 5-day forecast for API city '{city_name_for_api_request}' (display to user: '{display_name_for_forecast_header}').")
    else:
