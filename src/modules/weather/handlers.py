...
async def _get_and_show_weather(
    ...
    if weather_data and (weather_data.get("cod") == 200 or str(weather_data.get("cod")) == "200"):
        ...
        try: await final_target_message.edit_text(text_to_send, reply_markup=reply_markup)
            ...
            except Exception as e2: logger.error(f"Failed to send final message either: {e2}")
         ...
         try: await final_target_message.edit_text(error_text, reply_markup=reply_markup)
             ...
             except Exception as e2: logger.error(f"Failed to send error message (404) either: {e2}")
...
async def handle_action_other_city(callback: CallbackQuery, state: FSMContext):
     logger.info(f"User {callback.from_user.id} requested OTHER city."); await callback.message.edit_text("🌍 Введіть назву іншого міста:", reply_markup=get_weather_enter_city_back_keyboard()); await state.set_state(WeatherStates.waiting_for_city); await callback.answer()
...
async def handle_action_refresh(callback: CallbackQuery, state: FSMContext, session: AsyncSession, bot: Bot):
     ...
     if coords: logger.info(f"User {user_id} requested REFRESH for coords: {coords}"); await _get_and_show_weather(bot, callback, state, session, coords=coords)
     elif city_name: logger.info(f"User {user_id} requested REFRESH for city: {city_name}"); await _get_and_show_weather(bot, callback, state, session, city_input=city_name)
...
async def handle_forecast_request(callback: CallbackQuery, state: FSMContext, session: AsyncSession, bot: Bot):
    ...
    if not city_name_for_request: logger.warning(f"User {user_id} requested forecast, no city in state."); await callback.answer("...", show_alert=True); return
...
async def handle_show_current_weather(callback: CallbackQuery, state: FSMContext, session: AsyncSession, bot: Bot):
     ...
     if current_city: logger.info(f"User {user_id} requested back to current weather: {current_city}"); await _get_and_show_weather(bot, callback, state, session, city_input=current_city)
